...
const g = require('../SlashCommands/genral/profile');
let background1 ="./SlashCommands/ggs.png"    
module.exports = {
  ...
  createFunCanvas:async(word,time,faster) => {
    ...
    ctx.font = "23px ahmed"
    ctx.fillStyle = 'black';
    ctx.fillText(word, canvas.width / 2, canvas.height / 3.2);
    ctx.font = "20px ahmed"
    ctx.fillText(faster, canvas.width / 2, 180);

    ctx.fillText(time, canvas.width / 1.8, 250);
    ctx.font = "18px ahmed" 
    ctx.beginPath();
